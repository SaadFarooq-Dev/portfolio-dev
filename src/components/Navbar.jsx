    ...
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary`}>
      <div className='w-full flex justify-between items-center max-2-7x1 mx-auto'>
        <Link className='flex items-center gap-2' onClick={() => {
          ...
          <p className='text-white text-[18px] font-bold cursor-pointer flex'>Saad &nbsp;
        ...
        <div className='sm:hidden flex flex-1 justify-end items-center'>
          ...
          <div className={`${!toggle ? 'hidden' : 'flex'} p-6 black-gradient absolute top-20 right-0 mx-4 my-2 min-w-[140px] z-10 rounded-xl`}>
            <ul className='list-none flex justify-end items-start flex-col gap-4'>
              {navLinks.map((link) => (
                <li key={link.id} className={`${active === link.title ? "text-white" : "text-secondary"} font-poppins font-meduim cursor-pointer text-[16px]`} onClick={() => {
                  setToggle(!toggle)
